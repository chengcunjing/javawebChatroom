...
/*					//测试 ServletContextAttributeListener attributeRemoved() 方法的触发条件
					application.setAttribute("test1", 1);
					application.removeAttribute("test1");
					
					//测试 ServletRequestAttributeListener attributeRemoved() 方法的触发条件
					request.setAttribute("test2", 1);
					request.removeAttribute("test2");
	
					//测试 HttpSessionAttributeListener attributeRemoved() 方法的触发条件
					session.setAttribute("test3", 1);
					session.removeAttribute("test3");
					
					//测试 HttpSessionBindingListener valueBound() 方法的触发条件
					session.setAttribute("test4", new TestHttpSessionBindingListener(loginmsg));
					//测试 HttpSessionBindingListener valueUnbound() 方法的触发条件
					session.removeAttribute("test4");
					
					//测试HttpSessionActivationListener 监听器   //序列化操作会在session失效时自动触发，反序列化需要自己去session对象中拿值
					//FIXME 从序列化的session中取出对象
					
					if(session.getAttribute("test5") == null){
						TestHttpSessionActivationListener  t = new TestHttpSessionActivationListener("orlando");
						session.setAttribute("test5", t);
					}
					System.out.println(session.getId() + session.getAttribute("test5"));*/
	...
	public void regedit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, SQLException {
		...
		userInfo.setUserEmail(email);
		userInfo.setUserPhone(phone);
		userInfo.setUserImg("images/u_icon.png");
		userInfo.setUserState(false);
